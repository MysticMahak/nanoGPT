...
def compute_kvs(model, tree, device):
    def dfs(node):
        if node.parent is None:
            ...
            idx = torch.tensor(node.tokens, dtype=torch.long, device=device).unsqueeze(0)

            _, _, kv_cache = model(idx, targets=None, kv_cache=node.parent.kvcache, pos_offset=node.parent.seq_len, is_prefill=True)

            node.kvcache = kv_cache
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, kv_cache=None, pos_offset=0, is_prefill=False):
        device = idx.device
        b, t = idx.size()
        ...
        if kv_cache is None:
            ...
            x = x[:, -1:, :] # if using a kv cache, only the last token gets fed forward at each step (see generate function), so crop context to that
        
        new_kv_cache = []
        for block, kv_cache_block in zip(self.transformer.h, kv_cache):
            x, new_kv_cache_block = block(x, kv_cache=kv_cache_block)
            new_kv_cache.append(new_kv_cache_block)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        return idx, average_time / max_new_tokens
